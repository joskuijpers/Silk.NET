...
namespace DemoRenderer.UI
{
    ...
    public class ImageRenderer(GL gl, ContentArchive content, int maximumGlyphsPerDraw = 2048)
        : Shader(gl, content.Load<GLSLContent>(@"UI\RenderImages.glvs").Source,
    ...
    )
    {
        ...
        private readonly ConstantsBuffer<VertexConstants> vertexConstants = new(gl, BufferTargetARB.UniformBuffer, debugName: "Image Renderer Vertex Constants");
        private readonly StructuredBuffer<ImageInstance> instances = new(gl, BufferTargetARB.ShaderStorageBuffer, maximumGlyphsPerDraw, "Image Instances");
        private readonly IndexBuffer indices = new(gl, Helpers.GetQuadIndices(maximumGlyphsPerDraw), "Image Indices");
        ...
        public void Render(RenderableImage image, Int2 screenResolution, Span<ImageInstance> instances)
        {
            ...
            while (count > 0)
            {
                ...
                gl.DrawElements(PrimitiveType.Triangles, (uint)batchCount * 6, indices.Type, 0);
                count -= batchCount;
                start += batchCount;
            }
        }

...
namespace DemoRenderer.UI
{
    ...
    public class Font : Disposable
    {
        ...
        public unsafe Font(GL gl, FontContent font)
        {
            this.gl = gl;
            Content = font;
            Sources = new StructuredBuffer<GlyphSource>(gl, BufferTargetARB.ShaderStorageBuffer, font.Characters.Count, font.Name + " Glyph Sources");

            gl.BindTexture(TextureTarget.Texture2D, Atlas = gl.GenTexture());
            var data = font.Atlas.Pin();
            for (int mipLevel = 0; mipLevel < font.Atlas.MipLevels; ++mipLevel)
            {
                gl.TexImage2D(GLEnum.Texture2D, mipLevel, (int)InternalFormat.R8SNorm, (uint)font.Atlas.Width >> mipLevel, (uint)font.Atlas.Height >> mipLevel, 0, PixelFormat.Red, PixelType.Byte, data + font.Atlas.GetMipStartIndex(mipLevel));
            }
            font.Atlas.Unpin();

...
namespace DemoRenderer.UI
{
    ...
    public class UILineRenderer(GL gl, ContentArchive content, int maximumLinesPerDraw = 2048)
        : Shader(gl, content.Load<GLSLContent>(@"UI\RenderUILines.glvs").Source,
    ...
    )
    {
        ...
        private readonly ConstantsBuffer<VertexConstants> vertexConstants = new(gl, BufferTargetARB.UniformBuffer, debugName: "UI Line Renderer Vertex Constants");
        private readonly StructuredBuffer<UILineInstance> instances = new(gl, BufferTargetARB.ShaderStorageBuffer, maximumLinesPerDraw, "UI Line Instances");
        private readonly IndexBuffer indices = new(gl, Helpers.GetQuadIndices(maximumLinesPerDraw), "UI Line Indices");

        public unsafe void Render(Int2 screenResolution, UILineInstance[] lines, int start, int count)
        {
            Use();
            indices.Bind();

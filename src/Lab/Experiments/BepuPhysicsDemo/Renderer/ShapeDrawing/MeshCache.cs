...
namespace DemoRenderer.ShapeDrawing
{
    ...
    public class MeshCache : Disposable
    {
        ...
        Allocator allocator;
        public MeshCache(GL gl, BufferPool pool, int initialSizeInVertices = 1 << 22)
        {
            Pool = pool;
            pool.TakeAtLeast(initialSizeInVertices, out vertices);
            TriangleBuffer = new StructuredBuffer<Vector4>(gl, BufferTargetARB.ShaderStorageBuffer, initialSizeInVertices, "Mesh Cache Vertex Buffer");

...
namespace Demos
{
    public class RayCastingDemo : Demo
    {
        ...
        public override void Update(Window window, Camera camera, Input input, float dt)
        {
            ...
            if (input.WasPushed(Key.R))
            {
                shouldRotate = !shouldRotate;
            }
            if (input.WasPushed(Key.F))
            {
                rotation = 0;
            }
            if (input.WasPushed(Key.T))
            {
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text, new Vector2(32, renderer.Resolution.Y - 192), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Demo specific controls:"), new Vector2(32, renderer.Resolution.Y - 176), 16, new Vector3(1), font);
            WriteControl("Change ray source", text.Clear().Append("1 through ").Append(raySources.Count), renderer.Resolution.Y - 160, renderer.TextBatcher, font);
            WriteControl("Toggle cycling", text.Clear().Append("C"), renderer.Resolution.Y - 144, renderer.TextBatcher, font);
            WriteControl("Toggle rotation", text.Clear().Append("R"), renderer.Resolution.Y - 128, renderer.TextBatcher, font);
            WriteControl("Reset rotation", text.Clear().Append("F"), renderer.Resolution.Y - 112, renderer.TextBatcher, font);
            WriteControl("Toggle threading", text.Clear().Append("T"), renderer.Resolution.Y - 96, renderer.TextBatcher, font);

            renderer.TextBatcher.Write(text.Clear().Append("Ray count: ").Append(testRays.Count), new Vector2(32, renderer.Resolution.Y - 80), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Time (us):"), new Vector2(128, renderer.Resolution.Y - 64), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Rays per second:"), new Vector2(224, renderer.Resolution.Y - 64), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Relative speed:"), new Vector2(350, renderer.Resolution.Y - 64), 16, new Vector3(1), font);
            ...
            for (int i = 0; i < algorithms.Length; ++i)
            {
                ...
                WriteResults(algorithms[i].Name, stats.Average, baseStats.Average, renderer.Resolution.Y - (baseHeight - 16 * i), renderer.TextBatcher, text, font);
            }
